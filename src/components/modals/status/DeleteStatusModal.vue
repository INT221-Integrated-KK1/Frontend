      ...
      <div class="flex items-center justify-center min-h-screen bg-black/[.05]">
        <div class="bg-white w-1/2 p-6 rounded shadow-lg">
          <div class="">
            ...
            >
              <option
                v-for="(status, index) in statusmanager.getStatus()"
                :key="index"
                :value="status.id"
              >
                {{ status.name }}
            ...
            <div class="text-right">
              <button
                class="itbkk-button-confirm btn bg-green-500 hover:bg-green-700 text-white mr-3"
                @click="transferConfirm(selectId)"
                :disabled="selectId === undefined"
              >
                Transfer
              </button>
              <router-link :to="{ name: 'status' }">
                <button
                  class="itbkk-button-cancel btn bg-red-500 hover:bg-red-700 text-white"
                  @click="$emit('close')"
                >
                  Cancel
                </button>
      ...
      <div class="flex items-center justify-center min-h-screen bg-black/[.05]">
        <div class="bg-white w-1/2 p-6 rounded shadow-lg">
          <div class="">
            ...
            <hr />
            <p class="text-lg py-2 mb-2 break-words">Do you want to confirm delete?</p>
